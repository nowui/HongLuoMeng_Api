...
public class CartDao {
	...
	private Integer count(Cart cart) {
		...
		StringBuffer sql = new StringBuffer("SELECT COUNT(*) FROM " + Cart.KEY_SHOP + " ");
		...
		sql.append(Cart.KEY_SHOP_STATUS + " = 1 ");
	...
	private List<Cart> list(Cart cart, Integer m, Integer n) {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM " + Cart.KEY_SHOP + " ");
		...
		sql.append("ORDER BY " + Cart.KEY_SHOP_CREATE_TIME + " DESC ");
	...
	public void update(Cart cart, String request_user_id) {
		cart.remove(Cart.KEY_SHOP_CREATE_USER_ID);
		cart.remove(Cart.KEY_SHOP_CREATE_TIME);
		cart.setCart_update_user_id(request_user_id);
		cart.setCart_update_time(new Date());
