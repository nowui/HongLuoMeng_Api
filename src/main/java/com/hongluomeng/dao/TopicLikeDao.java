...
public class TopicLikeDao extends BaseDao {
	...
	private List<TopicLike> list(TopicLike topicLike, Integer m, Integer n) {
        ...
        myDynamicSQL.isNullOrEmpty("AND " + TopicLike.TABLE_TOPIC_LIKE + "." + TopicLike.COLUMN_USER_ID + " = ? ", topicLike.getUser_id());
    ...
    private TopicLike find(TopicLike topicLike) {
        MyDynamicSQL myDynamicSQL = new MyDynamicSQL();

        myDynamicSQL.append("SELECT * FROM " + TopicLike.TABLE_TOPIC_LIKE + " ");
        myDynamicSQL.append("WHERE " + TopicLike.TABLE_TOPIC_LIKE + "." + TopicLike.COLUMN_SYSTEM_STATUS + " = 1 ");
        myDynamicSQL.isNullOrEmpty("AND " + TopicLike.COLUMN_TOPIC_ID + " = ? ", topicLike.getTopic_id());
        myDynamicSQL.isNullOrEmpty("AND " + TopicLike.COLUMN_USER_ID + " = ? ", topicLike.getUser_id());

        List<TopicLike> topicLikeList = new TopicLike().find(myDynamicSQL.sql.toString(), myDynamicSQL.parameterList.toArray());
        if(topicLikeList.size() == 0) {
            return null;
        } else {
            return topicLikeList.get(0);
        }
    }

    public TopicLike findByTopic_idAndUser_id(String topic_id, String user_id) {
        TopicLike topicLike = new TopicLike();
        topicLike.setTopic_id(topic_id);
        topicLike.setUser_id(user_id);

        return find(topicLike);
    }

	public void save(TopicLike topicLike, String request_user_id) {
        TopicLike tl = findByTopic_idAndUser_id(topicLike.getTopic_id(), request_user_id);

        if (tl == null) {
            topicLike.setTopic_like_id(Utility.getUUID());
            ...
            topicLike.setSystem_status(true);

            topicLike.initForUpdate(request_user_id);
