	...
	protected void validate(Controller controller) {
		...
		switch (actionKey) {
			...
			case Url.URL_MENU_UPDATE:
				...
				if (actionKey.equals(Url.URL_MENU_SAVE) && Utility.isNullOrEmpty(category.getParent_id())) {
					message += "父编号为空";
					message += Const.LINE_FEED;
				}

				if (actionKey.equals(Url.URL_MENU_UPDATE) && Utility.isNullOrEmpty(category.getCategory_id())) {
					message += "编号为空";
					message += Const.LINE_FEED;
				}
