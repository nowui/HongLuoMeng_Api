...
public class MemberService {
	...
	public Map<String, Object> save(JSONObject jsonObject) {
		...
		Integer count = smsService.countBySms_phoneAndSms_codeAndMinute(userMap.getUser_phone(), sms.getSms_code(), 30);

		if (count == 0) {
			throw new RuntimeException("验证码已经过期");
		}

		smsService.updateSms_statusBySms_phone(userMap.getUser_phone(), sms.getSms_code());
	...
	/*public void update(JSONObject jsonObject) {
		...
		memberDao.update(memberMap, jsonObject.getString(Const.KEY_REQUEST_USER_ID));
	}
